...
def device2():
	...
	for i in range(8, 16):
		...
		# !!!  晚上24点前用！否则刷新！  !!!
		print("晚上7点后用的，自动收集红色碎片")
		d1.autoCollectRedPieces()

		print("晚上7点后用的，自动收集军团的物资")
		d1.autoCollectClubBonus()

		print("自动完成任务，一天一次，晚上7点后完成")
		if i in [1, 13]:
			print(i, " skip")
			continue
		d1.autoFinishMission()
...
def main():
	...
	# for i in range(1, 8):
	# 	switchAccount(i)
	t1 = Thread(target = device1)
	t1.start()

	t2 = Thread(target = device2)
	t2.start()

	t3 = Thread(target = device3)
	t3.start()
